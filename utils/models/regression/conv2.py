...
def get_model(end_label = True):
    ...
    x     = layers.Conv2D(200, kernel_size=(1,10), padding='same', activation='elu', kernel_regularizer=regularizers.l2(l2))(x)
    x     = layers.BatchNormalization()(x)
    x     = layers.AveragePooling2D(pool_size=(1,10))(x)

    x     = layers.Conv2D(200, kernel_size=(1,1), padding='same', activation='elu', kernel_regularizer=regularizers.l2(l2))(x)
    x     = layers.BatchNormalization()(x)
    ...
    if end_label:
        ...
        degree_output = layers.Dense(1, activation = 'tanh')(x)
    else:
        distance_output = layers.Dense(500, activation = 'sigmoid')(x)
        degree_output = layers.Dense(500, activation = 'tanh')(x)
